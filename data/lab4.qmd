...
# Contingency table
cont_table2 <- table(sal_clean$section, sal_clean$unittype)
print(cont_table)

# Chi-square Test
chisq.test(cont_table2)

# The p-value is less than 0.05 (2.2e-16), which means that there is a statistically significant relationship between forest type and channel unit in regards to coastal giant salamander abundance. This means we reject the null hypothesis.
```
```{r}
...
box_plot <- sal_clean %>% 
  ...
  ggpubr::ggbarplot(x = 'unittype', y = 'n',
                    ...
                    add = "mean_se")
ggsave("sal_chi_test.png", plot = box_plot, width = 10, height = 6, dpi = 300)
print(box_plot)
...
# Equal variance test
cc_sal_weight <- sal_clean %>% 
  filter(section == "CC") %>% 
  pull(weight_g)

og_sal_weight <- sal_clean %>% 
  filter(section == "OG") %>% 
  pull(weight_g)

var.test(cc_sal_weight, og_sal_weight)

# The results of this test suggests the variances are not equal. We know this because the p-value is less than 0.05 (1.439e-07). This means we reject the null hypothesis that the variances are equal.
```
```{r}
str(cc_sal_weight)
str(og_sal_weight)
...
ggpubr::ggarrange(ggpubr::gghistogram(cc_sal_weight, main = "Clear Cut"),
                                      ggpubr::gghistogram(og_sal_weight, main = "Old Growth"))

# These are right skewed, so we have to either do a log transform to normalize the data, or use a Welch t-test.
```

```{r}
# Log transform
var.test(log(cc_sal_weight), log(og_sal_weight))

# The p-value is still less than 0.05 (0.005299), so we reject the null hypothesis, meaning that the data is not normally distributed. I will use the Welch t-test to continue.
```
...
# The p-value is less 0.05 (8.629e-07), so we reject the null hypothesis, meaning that the coastal giant salamander did not have the same weight in the two forests. By looking at the means, we can see the salamander had significantly higher weights in clear cut forests than in old growth forests.
```
...
```{r}
# Find the length variable
?and_vertebrates

# length_1_mm is what we are looking for
```
...
# Both are skewed and not normally distributed. To statistically test if a variable fits a normal distribution, we will use the Shapiro-Wilk normality test below.
```

```{r}
# Shapiro-Wilk normality test only runs for 5000 observations below, so we are adjusting to that and then conducting the test.
...
# The p-value is less than 0.05 for both (2.2e-16), so we reject the null hypothesis, meaning that our data does not fit a normal distribution. So we can either use log transform and use the Pearson's correlation test or we can use the Spearman correlation test. 
```
...
# This is not normally distributed, so I will use the Spearman correlation test.
```
...
# The p-value is less than 0.05 (2.2e-16), so we reject the null, meaning the data represents a significant, positive relationship between length and weight for the Cutthroat trout. There is also a very high correlation coefficient
```
...
# The results of the ANOVA test are highly significant, meaning that at least one site has significantly different crab sizes than the other sites. We know this because the p-value is less than 0.05 (2.2e-16), leading us to reject the null hypothesis.

# The Tukey's HSD Test shows us that the site relationships: GTM-DB, PIE_DB, and PIE-GTM are statistically significant, because their p-values are all less than 0.05. These relationships being statistically significant means that they have significant difference from each other.
```
...
# The p-value is greater than 0.05, which means we fail to reject the null hypothesis. This means that there is not enough statistical evidence to represent a significant relationship between water_temp_sd and size.
```
...
# air_temp_sd and latitude have a higher correlation coefficient, with it being greater than 0.7 (0.79321301), but water_temp_sd and air_temp_sd, and latitude and water_temp_sd, are not highly correlated. It is worse if the variables are highly correlated because they do increase the complexity of the linear regression model. 
```
...
# The overall p-value is less than 0.05 (2.2e-16), which means that there is a significant impact of the combination of predictors on crab size. The individual p-values are a bit of a different result. Latitude is the only one less than 0.05, meaning it is the only statistically significant relationship with an effect on crab size. air_temp_sd and water_temp_sd both have p-values greater than 0.05, meaning we fail to reject the null hypothesis. This means there is not enough statistical evidence to represent a statistically significant relationship between these two predictors and crab size.
```
